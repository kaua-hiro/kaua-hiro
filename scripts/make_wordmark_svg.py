...
# Windows has no Futura; Arial Bold is the substitute (index 0 -- plain .ttf, not
# a .ttc face collection). On macOS, swap back to Futura via WORDMARK_FONT.
...
TEXT = os.environ.get("WORDMARK_TEXT", "KAUA")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">kaua-hiro@github: ~$ ./wordmark.sh --3d</text>')
...
def main():
    ...
    out = a.out or os.path.join(HERE, "..", f"kaua-wordmark-{a.mode}.svg" if a.mode != "rock" else "kaua-wordmark.svg")
